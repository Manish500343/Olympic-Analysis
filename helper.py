...
def fetch_medal_tally(df, year, country):
    ...
    if flag == 1:
        x = medal_df.groupby('Year')[['Gold', 'Silver', 'Bronze']].sum().sort_values('Year').reset_index()
    else:
...
def yearwise_medal_tally(df,country):
    tempp_df = df.dropna(subset=['Medal'])
    tempp_df.drop_duplicates(subset=['Team', 'NOC', 'Games', 'Year',
                                   'City', 'Sport', 'Event', 'Medal'], inplace=True)
    new_df = tempp_df[tempp_df['region'] == country]
...
def men_vs_women(df):
    ...
    final.rename(columns={'Name_x':'Male',
                          'Name_y':'Female'},inplace=True)
