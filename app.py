...
if user_menu=='Country-wise Analysis':
    ...
    country_list.sort()
    choosen_country=st.sidebar.selectbox('Seclect Country', country_list)
    yearwise_medal=helper.yearwise_medal_tally(df, str(choosen_country))
    st.title(choosen_country+" Medal Tally over the years")
    fig=px.line(yearwise_medal, x='Year', y='Medal')
    st.plotly_chart(fig)

    st.title(choosen_country+" excels in the following sports")
    pt=helper.country_event_heatmap(df, choosen_country)

    fig,ax=plt.subplots(figsize=(20,20))
    ax=sns.heatmap(pt,annot=True)
    st.pyplot(fig)

    st.title("Top 10 athletes of "+choosen_country)
    top10_df=helper.most_successful_countrywise(df,choosen_country)
    st.table(top10_df)
...
if user_menu == 'Athelete wise Analysis':
    ...
    fig=ff.create_distplot([x1,x2,x3,x4],['Overall Age',
                                          'Gold Medalist',
                                          'Silver Medalist',
                                          'Bronze Medalist'],show_rug=False,show_hist=False)
